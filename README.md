...
The fine-tuned model improves severity classification accuracy from **13.5% (zero-shot)** to **85.0%** on a held-out HAM10000 test set, and is quantized to a **4.36 GB GGUF artifact** that runs fully offline on an 8GB consumer GPU.
...
|---|---|---|
| Severity accuracy | 13.5% (95% CI: 9.0–18.5%) | **85.0%** (95% CI: 79.5–90.0%) |
| Cohen's Kappa | 0.014 | **0.608** |
| Matthews Correlation | 0.030 | **0.631** |
| RMSE (severity ordinal) | 1.505 | **0.693** |
...
**Per-severity-tier performance:**

| Severity | Zero-Shot P / R / F1 | Fine-tuned P / R / F1 |
|---|---|---|
| LOW | 0.200 / 0.006 / 0.012 | **0.986 / 0.865 / 0.922** |
| MEDIUM | 0.061 / 0.556 / 0.110 | **0.545 / 0.667 / 0.600** |
| HIGH | 0.177 / 0.714 / 0.284 | **0.500 / 0.821 / 0.622** |

HIGH-severity recall (0.821) is the most clinically important number here -- the fine-tuned model rarely misses a true high-severity case, but at the cost of precision (more false positives flagged as HIGH than ideal). This is the safer failure direction for a triage tool, but it's a tradeoff worth stating plainly rather than hiding behind the aggregate accuracy figure. The zero-shot baseline's LOW recall of 0.006 confirms it isn't doing structured triage at all -- it's defaulting to MEDIUM/HIGH almost regardless of input. MEDIUM has only 9 support samples in this n=200 split -- treat that row as a low-confidence estimate.
...
This project intentionally reports negative findings alongside the headline result, a full writeup is yet to be made.
...
- **Federated LoRA aggregation is a proposed direction, not implemented.** No FedAvg, multi-clinic split, or adapter-aggregation code exists in this repository yet. The architecture (averaging LoRA adapter weights across simulated clinic nodes, given the small adapter size relative to full fine-tuning) is sound on paper but unvalidated -- treat any mention of it elsewhere as a design proposal for future work, not a completed or partial contribution.
- **Evaluation is on n=200** of the 3,005-sample held-out set. 95% CIs (bootstrap, n=1000, regenerated in Cell 10a, see `metrics.json`) are reasonably wide at this sample size (accuracy 79.5–90.0%) -- full-set evaluation would tighten these but wasn't run due to compute time.
...
- [ ] Implement and validate Federated LoRA (FedAvg) aggregation across simulated clinic nodes -- currently a design proposal only, no code exists
